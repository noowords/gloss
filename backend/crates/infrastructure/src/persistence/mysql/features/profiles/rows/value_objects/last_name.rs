use domain::aggregates::users::profile::value_objects::{ ProfileLastName };

#[derive(Debug, Clone, PartialEq, Eq, sqlx::Type, sqlx::FromRow)]
#[sqlx(transparent)]
pub struct MySqlProfileLastNameRow(String);

impl From<MySqlProfileLastNameRow> for String {
    fn from(row: MySqlProfileLastNameRow) -> Self {
        row.0
    }
}

impl From<String> for MySqlProfileLastNameRow {
    fn from(value: String) -> Self {
        Self(value)
    }
}

impl From<MySqlProfileLastNameRow> for ProfileLastName {
    fn from(row: MySqlProfileLastNameRow) -> Self {
        row.0.try_into().expect("Invalid profile last name")
    }
}

impl From<ProfileLastName> for MySqlProfileLastNameRow {
    fn from(entity: ProfileLastName) -> Self {
        Self(entity.into())
    }
}
